...
const CreateCategory = () => {
  ...
  const deleteCategoryHandler=async(categoryName,categoryId)=>{
    try{
      const response=await axios.delete(`http://localhost:8080/api/v1/category/delete-category/${categoryId}`);
  ...
  const createNewCategoryHandler=async()=>{
	 ...
	 {
		const res=await axios.post('http://localhost:8080/api/v1/category/create-category',{name:newCategory});
		if(res.data.success)
		{
	...
	if(ok)
	{
	   ...
	   try {
		const { data } = await axios.put(
		  `http://localhost:8080/api/v1/category/update-category/${selectedCategory.id}`,
		  { name: updatedValue }
		);
		if (data?.success) {
			...
			setOk(false);
			toast.success(`${selectedCategory.name} is updated to ${updatedValue}`);
			getAllCategories();
		} else {
  ...
  return (
    <Layout title="create-category">
      ...
      <div className="row">
        ...
        <div className="col-md-9">
          ...
          <div className="p-3 w-50 mb-3">
            ...
            />
            <button type="button" class="btn btn-primary mt-3" onClick={createNewCategoryHandler}>
              Submit
            </button>
          </div>
          <div>
              ...
              <tbody>
				{
					categories.map((category,index)=>{
						return <tr>
							<td>{category.name}</td>
							<td><button onClick={()=>editCategoryHandler(category._id,category.name)} type="button" class="btn btn-primary">Edit</button> <button type="button" onClick={()=>deleteCategoryHandler(category.name,category._id)} class="btn btn-danger">Delete</button></td>
	  ...
	  <Modal title="Update Category" open={isModalOpen} onOk={saveUpdatedValueToDb} onCancel={()=>{setUpdatedValue('');
	  setIsModalOpen(false)}}>
        <input
		 value={updatedValue}
		      minLength={3}
